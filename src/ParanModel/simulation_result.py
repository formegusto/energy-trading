...
def simulation_result(self, kwh=None, public_percentage=30):
    ...
    self.after_households = np.array([Household(name, kwh, self.simulation_datas['거래 이익'].values[idx]).set_rate("단일", "기타")
                                      for idx, (name, kwh) in enumerate(self.datas.iloc[:, :3].to_numpy())])
